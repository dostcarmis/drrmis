    ...
    <div class="container-fluid" id="clear-fluid">
        <div class="wrap">
            ...
            <div class="mt-5 d-none" id="report-view-div">{{-- REPORT --}}
                <a id="r-anchor">
                    <h4 style="padding-top: 60px; margin-top: -60px; color: black">Generated Report</h4>
                </a>
                <div class="row">
                    <div class="col-sm-6">
                ...
                <div class="d-none">
                    <iframe name= "d-none print_frame" frameborder="0"></iframe>
                ...
                <div class="form-group px-3">
                    <form id="clears-edit-form">
                        ...
                        <fieldset>
                            ...
                            <input type="hidden" name="del-image" id="form-c-del-image" value=false>
                        </fieldset>
                        <fieldset class="mt-3">
...
<script>
    ...
    .on('click','.clears-e-btn',function(e){
        ...
        if( img != '' && img != null && img != undefined){
            $('#form-image-holder').removeClass('d-flex').hide()
            $('.form-c-res-image-btn').attr('default',"url({{asset('photos/clears')}}"+"/"+img+")");
            $('#clears-edit-form #c-image-preview').show();
            $('#clears-edit-form #c-image-preview').css('background-image',"url({{asset('photos/clears')}}"+"/"+img+")");
        }else{
            $('#form-c-image').val('');
            $('#form-image-holder').addClass('d-flex').show()
            $('#clears-edit-form #c-image-preview').hide();
        }
    ...
    .on('click','#form-c-del-image-btn', function(e){
        ...
        if(confirm("Are you sure you want to remove the image?")){
            ...
            $('#form-c-del-image').val(true);
            $('#form-image-holder').addClass('d-flex').show();
            $('#c-image-preview').hide();
        }
            ...
            success:function(response){
                if($.isEmptyObject(response.error)){
                    if(response.success){
                        ...
                        $('#form-c-del-image').val(false);
                        setTimeout(function(){
                            $('#alert-update-1').fadeOut();
                        },2000)
                    }else{
                        if(response.msg){
                            $('.alert').hide();
    ...
    .on('click','.c-report',function(e){
        ...
        window.location= '#r-anchor' //'#report-view-div';
    })
